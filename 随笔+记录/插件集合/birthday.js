(function (window, undefined) {
	...
	Birth.func = function(cla,type) {
		...
		switch(_type) {
			case 'year':
				var d     = new Date()
				var _yearEnd = d.getFullYear();
				var _yearStart = _yearEnd - 100;
		...
		$('.year-month .enter').bind('click', function(event) {
			var _a = $('#year').find('.active').html();
			var _b = $('#month').find('.active').html();
			// if(_b<10){
			// 	_b="0"+_b;
			// }
			// console.log(_b)
			$('#birth').val(_a+_b);
			$(this).parents(".year-month").siblings("p").css("display","block");
			// console.log(_a+_b)
			Birth.hide();
			event.preventDefault();
		});
